...
For a while now, I have been using [Andrew
Heiss’](https://www.andrewheiss.com/) custom LaTeX templates
...
between both the hikmah and hikmah-manuscript variant. In the future I
hope to add a few custom additions to this.

There are two formats that can be applied:

- By default, authors are laid out in a 3x1 grid (Shifu Typst
  (default)).
- However, I found this messy with 4+ authors so by setting
  `block-author=false` inline authorship is applied, typical with what
  you would see in journal style papers (Shifu Typst (custom))
