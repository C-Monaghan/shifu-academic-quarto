...
% Utility package for patching LaTeX commands
\usepackage{etoolbox}

% LEFT-ALIGNED TEXT OPTION
% ========================
% Provides ragged-right (left-aligned) text instead of fully justified
$if(left-aligned)$
\usepackage[document]{ragged2e}  % Apply ragged-right to entire document

% Configure ragged2e settings
\setlength{\RaggedRightParindent}{\parindent}  % Maintain normal paragraph indentation
\setlength{\RaggedRightRightskip}{0pt plus 3em}  % Allow some stretch for better appearance

% Fix indentation in specific environments that shouldn't have paragraph indentation
\AtBeginEnvironment{figure}{\setlength{\RaggedRightParindent}{0pt}}    % Figures
\AtBeginEnvironment{table}{\setlength{\RaggedRightParindent}{0pt}}     % Tables
\AtBeginEnvironment{longtable}{\setlength{\RaggedRightParindent}{0pt}} % Long tables
\AtBeginEnvironment{apptbl}{\setlength{\RaggedRightParindent}{0pt}}    % Custom appendix tables
\AtBeginEnvironment{tcolorbox}{\setlength{\RaggedRightParindent}{0pt}} % Quarto callout boxes
$endif$


% ENDNOTES OPTION
% ===============
% Converts footnotes to endnotes (collected at end of document)
$if(endnotes)$
\usepackage{endnotes}
\renewcommand{\enotesize}{\normalsize}  % Endnote text size
\let\footnote=\endnote  % Redirect all footnotes to become endnotes
$endif$
...
% Normal page style (used for most pages)
% Format: SHORT TITLE [right-aligned]          Page # [right-aligned]
...
% ===============================
% Process Quarto YAML metadata into LaTeX title block commands
\usepackage{authblk}  % Multiple author and affiliation support
...
% Document title from Quarto YAML
$if(title)$
